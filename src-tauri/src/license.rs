...
//!
//! Priority:
//!   1. Valid Keyforge subscription → Pro  (50 panes)
//!   2. 14-day in-app trial        → Trial (9 panes)
//!   3. Free tier                  → Free  (3 panes)
//!
//! Offline grace: 14 days from last successful Keyforge validation.
//! Trial: machine-bound, integrity-protected, no account needed.
...
/// How many days the app works offline using a cached valid validation.
const OFFLINE_GRACE_DAYS: i64 = 14;

/// Trial length in days.
const TRIAL_DAYS: i64 = 14;

/// Internal salt for trial integrity verification.
const TRIAL_SALT: &[u8] = b"codegrid-trial-integrity-2026";

pub const MAX_PANES_FREE: u32 = 3;
pub const MAX_PANES_TRIAL: u32 = 9;
...
impl LicenseStatus {
    ...
    pub fn trial(days_remaining: i64) -> Self {
        LicenseStatus {
            is_licensed: false,
            is_trial: true,
            trial_days_remaining: days_remaining,
            license_key: None,
            max_panes: MAX_PANES_TRIAL,
            subscription_expires_at: None,
            keyforge_status: None,
            is_offline_grace: false,
        }
...
/// Stable machine identifier: SHA-256 of (hardware UUID + username).
...
// ── Sync helpers — used for pane creation (no async) ─────────────────────────

/// Read cached license/trial state from DB without a network call.
pub fn get_cached_status(db: &crate::db::Database) -> LicenseStatus {
    // 1. Check Keyforge Pro
    if let Some(pro) = get_cached_pro(db) {
        return pro;
    }
    // 2. Check trial
    get_trial_status(db)
...
fn get_cached_pro(db: &crate::db::Database) -> Option<LicenseStatus> {
    let key = db.get_setting("keyforge_license_key").filter(|k| !k.is_empty())?;
    ...
    Some(LicenseStatus::pro_tier(&key, expires_at, false))
}

fn get_trial_status(db: &crate::db::Database) -> LicenseStatus {
    ...
    let first_launch_date = if let Some(ref date_str) = first_launch {
        // Verify integrity
...
fn is_within_grace_period(last_validated: &str) -> bool {
    ...
    (chrono::Utc::now() - last.with_timezone(&chrono::Utc)).num_days() < OFFLINE_GRACE_DAYS
}
...
pub async fn refresh_license_status(db: &crate::db::Database) -> LicenseStatus {
    ...
    match keyforge_validate(&key, &device_id).await {
        Ok(resp) => {
            ...
            let status_str = resp.get("status").and_then(|v| v.as_str()).unwrap_or("").to_string();
            let expires_at = resp
                .get("license").and_then(|l| l.get("expiresAt"))
        ...
        Err(_) => {
            // Network failure — try cached Pro, then trial
            if let Some(pro) = get_cached_pro(db) {
                return LicenseStatus { is_offline_grace: true, ..pro };
            }
            get_trial_status(db)
        }
    }
...
pub async fn activate_license_key(key: &str, db: &crate::db::Database) -> Result<LicenseStatus, String> {
    let device_id = get_machine_id();
    ...
    if !is_valid {
        ...
        return Err(match code {
            ...
            "license_expired" => "License expired. Renew at codegrid.app/pricing.".to_string(),
            "max_devices_reached" => "Device limit reached. Manage devices at keyforge.dev/portal/request.".to_string(),
            _ => "Activation failed. Check your key and try again.".to_string(),
        });
    }

    let status_str = resp.get("status").and_then(|v| v.as_str()).unwrap_or("active");
    let expires_at = resp.get("license").and_then(|l| l.get("expiresAt"))
        .and_then(|v| v.as_str()).map(|s| s.to_string());
    let now = chrono::Utc::now().to_rfc3339();
...
pub async fn deactivate_license_key(db: &crate::db::Database) -> LicenseStatus {
    if let Some(key) = db.get_setting("keyforge_license_key").filter(|k| !k.is_empty()) {
        let device_id = get_machine_id();
...
async fn keyforge_validate(key: &str, device_id: &str) -> Result<serde_json::Value, reqwest::Error> {
    let client = reqwest::Client::builder().timeout(std::time::Duration::from_secs(10)).build()?;
    let resp = client.post(format!("{KEYFORGE_API}/licenses/validate"))
        .json(&serde_json::json!({ "licenseKey": key, "deviceIdentifier": device_id, "productId": KEYFORGE_PRODUCT_ID }))
        .send().await?;
    Ok(resp.json().await.unwrap_or(serde_json::Value::Null))
}

async fn keyforge_activate(key: &str, device_id: &str, device_name: &str) -> Result<serde_json::Value, reqwest::Error> {
    let client = reqwest::Client::builder().timeout(std::time::Duration::from_secs(10)).build()?;
    let resp = client.post(format!("{KEYFORGE_API}/licenses/activate"))
        .json(&serde_json::json!({ "licenseKey": key, "deviceIdentifier": device_id, "deviceName": device_name, "productId": KEYFORGE_PRODUCT_ID }))
        .send().await?;
    Ok(resp.json().await.unwrap_or(serde_json::Value::Null))
}
...
async fn keyforge_deactivate(key: &str, device_id: &str) -> Result<(), reqwest::Error> {
    let client = reqwest::Client::builder().timeout(std::time::Duration::from_secs(10)).build()?;
    client.delete(format!("{KEYFORGE_API}/licenses/device"))
        .json(&serde_json::json!({ "licenseKey": key, "deviceIdentifier": device_id, "productId": KEYFORGE_PRODUCT_ID }))
        .send().await?;
    Ok(())
}
...
fn get_hardware_uuid() -> Option<String> {
    Some(std::env::var("COMPUTERNAME").or_else(|_| std::env::var("HOSTNAME")).unwrap_or_default())
}
